...
typedef enum {
    TEST      = 1 << 0,        // 0 -> NO TEST  | 1 -> TEST
    BENCH     = 1 << 1,        // 0 -> NO BENCH | 1 -> BENCH
    EXEC      = 1 << 2,        // 0 -> NO CASE EXEC | 1 -> CASE EXEC
    LIST      = 1 << 4,        // 0 -> NOLIST | 1 -> LIST
    SUITE     = 1 << 5,        // 0 -> NO SUITE | -> SUITE
} skester_cmd_flags;
...
typedef struct {
    uint8_t flags;
    uint8_t flags_order[_SKS_MAX_FLAGS];
...
void sks_pipe_new_flag_segment(skester_pipe* pipe, int* cases_index, int* suites_index, const int flag_order_index) {
     if (flag_order_index == 0) 
         return;

     uint8_t last_flag = pipe->flags_order[flag_order_index-1];

     if (last_flag == SUITE || last_flag == LIST) {
...
skester_pipe sks_parse_args(int argc, char** argv) {
    ...
    for (int x = 1; x < argc; x++) {
        if (strcmp(argv[x], "test") == 0) {
            ...
            pipe.flags |= TEST;
            pipe.flags |= EXEC;
            ADD_FLAG_ORDER(pipe, idx_flag_order, TEST);
        ...
        if (strcmp(argv[x], "bench") == 0) {
            ...
            pipe.flags |= BENCH;
            pipe.flags |= EXEC;
            ADD_FLAG_ORDER(pipe, idx_flag_order, BENCH);
        ...
        if (strcmp(argv[x], "list") == 0) {
            ...
            pipe.flags |= LIST;
            ADD_FLAG_ORDER(pipe, idx_flag_order, LIST);
        ...
        if (strcmp(argv[x], "-s") == 0) {
            ...
            pipe.flags |= SUITE;
            ADD_FLAG_ORDER(pipe, idx_flag_order, SUITE);
